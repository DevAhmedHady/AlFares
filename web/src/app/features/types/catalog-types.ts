...
@Component({ standalone: true, imports: [FormsModule, ButtonModule, DialogModule, InputTextModule, SelectModule, TagModule], template: `
  ...
  <div class="report-panel">@if(items().length){<table class="data-table"><thead><tr><th>الاسم</th>@if(expense){<th>النطاق</th>}<th>الحالة</th><th>إجراءات</th></tr></thead><tbody>@for(item of items();track item.id){<tr><td><strong>{{item.name}}</strong></td>@if(expense){<td>{{scopeLabel(item)}}</td>}<td><p-tag [severity]="item.isActive?'success':'secondary'" [value]="item.isActive?'نشط':'متوقف'"/></td><td><div class="row-actions"><p-button icon="pi pi-pencil" ariaLabel="تعديل النوع" [rounded]="true" [text]="true" (onClick)="edit(item)"/><p-button icon="pi pi-trash" ariaLabel="حذف النوع" severity="danger" [rounded]="true" [text]="true" (onClick)="remove(item)"/></div></td></tr>}</tbody></table>}@else{<div class="empty-panel"><i class="pi pi-tags"></i><p>لا توجد أنواع مسجلة حتى الآن.</p></div>}</div></section>
  <p-dialog [visible]="show()" (visibleChange)="show.set($event)" [modal]="true" [draggable]="false" [style]="{width:'min(500px, calc(100vw - 2rem))'}" [header]="editing() ? 'تعديل النوع' : 'إضافة نوع'"><div class="form-grid"><div class="field span-2"><label for="catalog-name">الاسم</label><input id="catalog-name" pInputText [ngModel]="form().name" (ngModelChange)="form.update(x=>({...x,name:$event}))"/></div>@if(expense){<div class="field span-2"><label for="catalog-scope">النطاق</label><p-select inputId="catalog-scope" class="w-full" [options]="scopes" optionLabel="label" optionValue="value" [ngModel]="form().scope" (ngModelChange)="form.update(x=>({...x,scope:$event}))" appendTo="body"/></div>}</div><ng-template pTemplate="footer"><div class="dialog-actions"><p-button label="إلغاء" severity="secondary" [text]="true" (onClick)="show.set(false)"/><p-button label="حفظ" icon="pi pi-check" [loading]="saving()" [disabled]="!form().name.trim()" (onClick)="save()"/></div></ng-template></p-dialog>` })
export class CatalogTypesComponent {
  private readonly http = inject(HttpClient); private readonly base = inject(API_BASE);
  readonly expense = inject(ActivatedRoute).snapshot.data['kind'] === 'expenses'; readonly items = signal<CatalogType[]>([]); readonly show = signal(false); readonly saving = signal(false); readonly editing = signal<CatalogType | null>(null); readonly form = signal({ name: '', scope: ExpenseScope.General, isActive: true }); readonly scopes = Object.entries(expenseScopeLabels).map(([k,v]) => ({label:v, value:+k}));
