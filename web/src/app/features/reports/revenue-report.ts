...
import { ChartSeries, ChartType, RevenueReportBreakdown, RevenueReportResponse, RevenueTypeResponse } from '../../core/models';
...
export class RevenueReportComponent {
  private readonly revenues = inject(RevenuesService);
  private readonly store = inject(AuthStore);

  readonly canExport = this.store.has('revenues.export');
  readonly ChartType = ChartType;
  readonly chartColors = ['#15803d', '#1e40af', '#0d9488', '#7c3aed', '#16a34a', '#0891b2', '#65a30d', '#059669'];
  ...
  readonly data = signal<RevenueReportResponse | null>(null);
  ...
  readonly categorySeries = computed<ChartSeries | null>(() => {
    ...
    return this.toSeries('الإيرادات حسب النوع', report.byCategory);
  });

  readonly monthSeries = computed<ChartSeries | null>(() => {
    const report = this.data();
    if (!report?.byMonth.length) return null;
    return this.toSeries('الإيرادات عبر الزمن', report.byMonth);
  });
  ...
  load(): void {
    ...
    this.revenues.report(this.requestBody()).subscribe({
      next: (report) => {
  ...
  exportAs(format: ExportFormat): void {
    ...
    this.revenues.reportExport(format, this.requestBody()).subscribe({
      next: (blob) => {
        const ext = format === ExportFormat.Xlsx ? 'xlsx' : 'pdf';
        downloadBlob(blob, `revenue-report.${ext}`);
  ...
  private requestBody() {
    if (this.usingMonthScope()) {
      return {
        ...
        year: this.forYear(),
        month: this.forMonth(),
        revenueTypeId: this.typeFilter() || null,
      };
    }
    return {
      from: this.from() || null,
  ...
  private toSeries(name: string, rows: RevenueReportBreakdown[]): ChartSeries {
