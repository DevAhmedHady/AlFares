...
@Component({ standalone: true, imports: [FormsModule, ButtonModule, DatePickerModule, SelectModule, TagModule], template: `
  ...
  <div class="report-panel"><div class="report-filters"><div class="field"><label>نوع الحساب</label><p-select [options]="owners" optionLabel="label" optionValue="value" [ngModel]="ownerType()" (ngModelChange)="ownerType.set($event)"/></div><div class="field"><label>{{ownerFieldLabel()}}</label><p-select [options]="entities()" optionLabel="label" optionValue="id" [filter]="true" filterBy="label" [ngModel]="ownerId()" (ngModelChange)="ownerId.set($event)" [loading]="loadingEntities()" [placeholder]="entityPlaceholder()" [emptyMessage]="entityPlaceholder()"/></div><div class="field"><label>من تاريخ</label><p-datepicker [ngModel]="fromModel()" (ngModelChange)="from.set(toIso($event))" dateFormat="dd/mm/yy" [showIcon]="true" iconDisplay="input" [showButtonBar]="true" appendTo="body" styleClass="w-full" inputStyleClass="w-full"/></div><div class="field"><label>إلى تاريخ</label><p-datepicker [ngModel]="toModel()" (ngModelChange)="to.set(toIso($event))" dateFormat="dd/mm/yy" [showIcon]="true" iconDisplay="input" [showButtonBar]="true" appendTo="body" styleClass="w-full" inputStyleClass="w-full"/></div></div><div style="margin-top:.8rem"><p-button label="عرض التقرير" icon="pi pi-search" [loading]="loading()" [disabled]="!ownerId()" (onClick)="load()"/></div></div>
  @if(data(); as report){<div class="summary-grid"><div class="summary-card positive"><span>إجمالي الإيرادات</span><strong>{{money(report.totalRevenues)}}</strong></div><div class="summary-card negative"><span>إجمالي المصروفات</span><strong>{{money(report.totalExpenses)}}</strong></div><div class="summary-card" [class.positive]="report.net>=0" [class.negative]="report.net<0"><span>صافي الحساب</span><strong>{{money(report.net)}}</strong></div></div><div class="report-panel">@if(report.entries.length){<table class="data-table"><thead><tr><th>التاريخ</th><th>الحركة</th><th>البيان</th><th class="num">المبلغ</th></tr></thead><tbody>@for(item of report.entries;track item.id){<tr><td>{{date(item.date)}}</td><td>@if(item.kind===Kind.Revenue){<p-tag severity="success" value="إيراد"/>}@else{<p-tag severity="danger" value="مصروف"/>}</td><td>{{item.description}}</td><td class="num" [class.amount-in]="item.kind===Kind.Revenue" [class.amount-out]="item.kind!==Kind.Revenue">{{item.kind===Kind.Revenue?'+':'−'}} {{money(item.amount)}}</td></tr>}</tbody></table>}@else{<div class="empty-panel"><i class="pi pi-inbox"></i><p>لا توجد حركات ضمن الفترة المحددة.</p></div>}</div>}@else if(loaded()){<div class="report-panel empty-panel"><i class="pi pi-search"></i><p>اختر حسابًا ثم اعرض التقرير.</p></div>}</section>` })
export class OwnerReportComponent {
  ...
  readonly ownerFieldLabel = computed(() => this.ownerType() === OwnerType.Client ? 'العميل' : 'السيارة');
  readonly entityPlaceholder = computed(() =>
    this.loadingEntities() ? 'جارٍ التحميل...'
    : this.entities().length ? `اختر ${this.ownerFieldLabel()}`
  ...
  constructor() {
    ...
    effect(() => { this.ownerType(); this.ownerId.set(''); this.data.set(null); this.loaded.set(false); this.loadEntities(); });
  }
  ...
  private loadEntities(): void {
    ...
    if (type === OwnerType.Client) {
      ...
      this.cars.grid(emptyGridQuery(500)).subscribe({
        next: page => this.entities.set(page.items.filter(c => c.type === carType)
          .map(c => ({ id: c.id, label: c.plateNumber ? `${c.name} · ${c.plateNumber}` : c.name }))),
        error: () => this.entities.set([]),
        complete: () => this.loadingEntities.set(false),
      });
    }
