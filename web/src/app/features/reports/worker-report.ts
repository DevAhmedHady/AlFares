  ...
  template: ` <section class="feature-page">
    <header class="feature-hero">
      <div class="feature-title">
        <span class="feature-icon"><i class="pi pi-id-card"></i></span>
        <div>
          <h1>تقرير العامل</h1>
          <p>كشف السلف والتسويات والرصيد المتحرك للعامل.</p>
        </div>
      </div>
    </header>
    <div class="report-panel">
      <div class="report-filters">
        <div class="field">
          <label for="worker-report-worker">العامل</label
          ><p-select
            class="w-full"
            inputId="worker-report-worker"
            [options]="workers()"
            optionLabel="label"
            optionValue="id"
            [filter]="true"
        ...
        <div class="field">
          <label for="worker-report-from">من تاريخ</label
          ><p-datepicker
            inputId="worker-report-from"
            [ngModel]="fromModel()"
        ...
        <div class="field">
          <label for="worker-report-to">إلى تاريخ</label
          ><p-datepicker
            inputId="worker-report-to"
            [ngModel]="toModel()"
    ...
    @if (data(); as report) {
      <div class="summary-grid">
        <div class="summary-card negative">
          <span>إجمالي السلف</span><strong>{{ money(report.totalAdvances) }}</strong>
        </div>
        <div class="summary-card positive">
          <span>إجمالي التسويات</span><strong>{{ money(report.totalSettlements) }}</strong>
        </div>
        <div class="summary-card" [class.negative]="report.balance > 0">
          <span>الرصيد الحالي</span><strong>{{ money(report.balance) }}</strong>
        </div>
      </div>
      <div class="report-panel">
        @if (report.transactions.length) {
          <table class="data-table">
            ...
            <tbody>
              @for (item of report.transactions; track item.id) {
                <tr>
                  <td>{{ date(item.date) }}</td>
                  <td>
                    @if (item.kind === 'تسوية') {
                      <p-tag severity="success" [value]="item.kind" />
                    } @else {
                      <p-tag severity="warn" [value]="item.kind" />
                    }
                  </td>
                  <td
                    class="num"
                    [class.amount-in]="item.kind === 'تسوية'"
                    [class.amount-out]="item.kind !== 'تسوية'"
                  >
                    {{ item.kind === 'تسوية' ? '−' : '+' }} {{ money(item.amount) }}
                  </td>
                  <td class="num">{{ money(item.runningBalance) }}</td>
                </tr>
              }
            </tbody>
          </table>
      ...
      <div class="report-panel empty-panel">
        ...
        <p>اختر عاملًا ثم اعرض التقرير.</p>
      </div>
    }
  </section>`,
})
export class WorkerReportComponent {
    ...
    this.workersApi.grid(emptyGridQuery(500)).subscribe({
      next: (page) =>
        this.workers.set(
          page.items.map((w) => ({
            id: w.id,
            label: w.jobTitle ? `${w.name} · ${w.jobTitle}` : w.name,
          })),
        ),
  ...
  load(): void {
    ...
    this.workersApi
      .report({ workerId: this.id(), from: this.from() || null, to: this.to() || null })
      .subscribe({
