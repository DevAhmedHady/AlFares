...
public sealed class TodosVerticalTests
{
    ...
    public void PastDueDate_Fails()
    {
        ...
        TodoItem.Create("x", today.AddDays(-1), TodoPriority.Normal, null, today)
    ...
    public async Task GridChart_Work()
    {
        ...
        var second = TodoItem.Create("عاجل", today.AddDays(2), TodoPriority.High, null, today).Value;
        ...
        var grid = await new GetTodosGridHandler(db).Handle(
            new(new GridQuery { Filters = [new("priority", GridFilterOp.Eq, "High")] }), default);
        grid.Value.Items.Should().HaveCount(2);

        var chart = await new TodosChartDataSource(db).ComputeAsync(
            new("priority", null, ChartAggregation.Count, []), default);
