...
public sealed class ClientsVerticalTests
{
    ...
    {
        await using var db = CreateDb(); var mapper = CreateMapper();
        var create = new CreateClientHandler(new ClientRepository(db), mapper);
        var created = await create.Handle(new CreateClientCommand("شركة الاختبار", "أحمد", "0100", "a@test.local", 500m, ActivityLevel.High, null), default);
        created.IsSuccess.Should().BeTrue();

        var grid = await new GetClientsGridHandler(db).Handle(new GetClientsGridQuery(new GridQuery
        { Filters = [new GridFilter("status", GridFilterOp.Eq, "Active")], Sort = [new GridSort("accountBalance", true)] }), default);
        grid.Value.Items.Should().ContainSingle().Which.Name.Should().Be("شركة الاختبار");

        var bytes = new ExcelGridExporter().Export(grid.Value.Items,
            [new ExportColumn(nameof(global::Clients.Contracts.ClientResponse.Name), "اسم العميل", GridFieldType.Text)], "العملاء");
