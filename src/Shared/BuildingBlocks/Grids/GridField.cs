...
public sealed record GridField(
    string Key,
    string DisplayName,
    GridFieldType Type,
    bool Searchable,
...
public sealed class GridFieldMap<T>
{
    ...
    public GridFieldMap(
        IEnumerable<(GridField Field, Expression<Func<T, object?>> Selector)> entries
    )
    {
        ArgumentNullException.ThrowIfNull(entries);
        var materialized = entries.ToArray();
        selectors = materialized.ToDictionary(
            x => x.Field.Key,
            x => x.Selector,
            StringComparer.OrdinalIgnoreCase
        );
        fieldsByKey = materialized.ToDictionary(
            x => x.Field.Key,
            x => x.Field,
            StringComparer.OrdinalIgnoreCase
        );
    ...
    public Expression<Func<T, object?>> Selector(string key) =>
        selectors.TryGetValue(key, out var selector)
            ? selector
            : throw new KeyNotFoundException($"Grid field '{key}' is not registered.");
