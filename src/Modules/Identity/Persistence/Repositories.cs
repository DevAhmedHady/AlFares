...
public sealed class UserRepository(IMainDbContext db) : IUserRepository
{
    ...
    public Task<User?> GetByEmailAsync(string normalizedEmail, CancellationToken ct) =>
        db.Set<User>()
            .FirstOrDefaultAsync(u => u.Email == Email.FromPersisted(normalizedEmail), ct);
...
{
    public Task<TenantUser?> GetActiveMembershipAsync(
        Guid tenantId,
        Guid userId,
        ...
        db.Set<TenantUser>()
            .FirstOrDefaultAsync(
                tu => tu.TenantId == tenantId && tu.UserId == userId && tu.IsActive,
                ct
            );
    ...
    {
        var membership = await db.Set<TenantUser>()
            .AsNoTracking()
            .FirstOrDefaultAsync(
                tu => tu.TenantId == tenantId && tu.UserId == userId && tu.IsActive,
                ct
        ...
        var roleIds = await db.Set<TenantUserRole>()
            .AsNoTracking()
        ...
        var roles = await db.Set<TenantRole>()
            .AsNoTracking()
            .Where(tr => roleIds.Contains(tr.Id))
            .Select(tr => tr.Name)
            .ToListAsync(ct);

        var permissionIds = await db.Set<TenantPermission>()
            .AsNoTracking()
    ...
    public Task<TenantRole?> GetTenantRoleByNameAsync(
        Guid tenantId,
        string name,
        CancellationToken ct
    ) =>
        db.Set<TenantRole>()
            .FirstOrDefaultAsync(tr => tr.TenantId == tenantId && tr.Name == name, ct);

    public Task<bool> HasTenantUserRoleAsync(
        Guid tenantUserId,
        Guid tenantRoleId,
        CancellationToken ct
    ) =>
        db.Set<TenantUserRole>()
            .AnyAsync(x => x.TenantUserId == tenantUserId && x.TenantRoleId == tenantRoleId, ct);
    ...
    public void AddTenantPermission(TenantPermission tenantPermission) =>
        db.Set<TenantPermission>().Add(tenantPermission);

    public void AddTenantUserRole(TenantUserRole tenantUserRole) =>
        db.Set<TenantUserRole>().Add(tenantUserRole);
