...
public static class IdentityErrors
{
    ...
    public static readonly Error SlugEmpty = Error.Validation(
        "Identity.Slug.Empty",
        "Slug is required."
    );
    public static readonly Error SlugInvalid = Error.Validation(
        "Identity.Slug.Invalid",
    ...
    public static readonly Error TenantNameEmpty = Error.Validation(
        "Identity.Tenant.NameEmpty",
        "Tenant name is required."
    );

    // Users / tenants
    public static readonly Error EmailTaken = Error.Conflict(
        "Identity.User.EmailTaken",
        "An account with this email already exists."
    );

    public static Error UserNotFound(Guid id) =>
        Error.NotFound("Identity.User.NotFound", $"User '{id}' was not found.");

    public static readonly Error SlugTaken = Error.Conflict(
        "Identity.Tenant.SlugTaken",
        "A tenant with this slug already exists."
    );

    public static Error TenantNotFound(Guid id) =>
        Error.NotFound("Identity.Tenant.NotFound", $"Tenant '{id}' was not found.");

    public static readonly Error OwnerRoleMissing = Error.Failure(
        "Identity.Tenant.OwnerRoleMissing",
        "Owner role template is not configured."
    );
