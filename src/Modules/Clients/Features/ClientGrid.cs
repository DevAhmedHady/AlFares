...
public static class ClientGrid
{
    ...
    public static readonly GridFieldMap<Client> Fields = new(
        new[]
        {
            (
                new GridField("name", "اسم العميل", GridFieldType.Text, true),
                (Expression<Func<Client, object?>>)(x => x.Name)
            ),
            (
                new GridField("contactName", "اسم جهة الاتصال", GridFieldType.Text, true),
                x => x.Contact.Name
            ),
    ...
    public static readonly Expression<Func<Client, ClientResponse>> Projection =
...
public sealed class GetClientsGridHandler(IMainDbContext dbContext)
    : IQueryHandler<GetClientsGridQuery, PagedResult<ClientResponse>>
{
    private readonly IMainDbContext dbContext =
        dbContext ?? throw new ArgumentNullException(nameof(dbContext));

    /// <inheritdoc />
    public async Task<Result<PagedResult<ClientResponse>>> Handle(
        GetClientsGridQuery query,
    ...
    )
    {
        var applied = dbContext
            .Set<Client>()
            .AsNoTracking()
            .ApplyGridQuery(query.Grid, ClientGrid.Fields);
        if (applied.IsFailure)
            return applied.Error;
        return await applied
            .Value.ToPagedResultAsync(query.Grid, ClientGrid.Projection, ct)
