...
public sealed class Contact : ValueObject
{
    ...
    public static Result<Contact> Create(string? name, string? phone, string? email)
    {
        if (string.IsNullOrWhiteSpace(name))
            return ClientErrors.ContactNameRequired;
        if (string.IsNullOrWhiteSpace(phone))
            return ClientErrors.PhoneRequired;
...
public sealed class Client : AggregateRoot
{
    ...
    )
    {
        ...
        if (balance < 0)
            return ClientErrors.BalanceInvalid;
    ...
    public Result<Client> SetStatus(ClientStatus status)
    {
        Status = status;
        UpdatedAtUtc = DateTime.UtcNow;
        return this;
